...
namespace K12.缺曠通知單
{
    public class Program
    {
        ...
        {
            string URL學生缺曠通知單 = "ischool/高中系統/共用/學務/學生/報表/缺曠通知單";
            string URL班級缺曠通知單 = "ischool/高中系統/共用/學務/班級/報表/缺曠通知單";

            FISCA.Features.Register(URL學生缺曠通知單, arg =>
            {
                if (K12.Presentation.NLDPanels.Student.SelectedSource.Count > 0)
                {
                    new K12.缺曠通知單.Report("student").Print();
                }
                else
                {
            ...
            FISCA.Features.Register(URL班級缺曠通知單, arg =>
            {
                ...
                {
                    new K12.缺曠通知單.Report("class").Print();
                }
                else
                {
            ...
            ribbon.Add(new RibbonFeature(Permissions.學生缺曠通知單, "缺曠通知單"));

            ribbon = RoleAclSource.Instance["班級"]["報表"];
            ribbon.Add(new RibbonFeature(Permissions.班級缺曠通知單, "缺曠通知單"));
        }
    }
}
