...
namespace K12.Behavior.DisciplineInput
{
    public partial class MerDemEditForm : FISCA.Presentation.Controls.BaseForm
    {
        ...
        Dictionary<string, string> DicDisciplineRegistByTeacher = new Dictionary<string, string>();
        int ConBoxIndex = 0;
        ...
        private void _loader_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            DicDisciplineRegistByTeacher.Clear();
            try
            {
                if (ConBoxIndex == 0) //缺曠日期
                {
                    DataTable dt = tool._Q.Select(@"select $td_take.* from $td_take join discipline on $td_take.ref_discipline_id = discipline.id where '" +
                        dateTimeInput1.Value.ToString("yyyy/MM/dd") + "' <= discipline.occur_date and discipline.occur_date <= '" + dateTimeInput2.Value.ToString("yyyy/MM/dd") + "'");
                    foreach (DataRow row in dt.Rows)
                    {
                        DicDisciplineRegistByTeacher.Add("" + row["ref_discipline_id"],""+row["ref_teacher_id"]);
                    }
                    List<string> tmp = new List<string>(DicDisciplineRegistByTeacher.Keys);
                    foreach (DisciplineRecord each in Discipline.SelectByOccurDate(list, dateTimeInput1.Value, dateTimeInput2.Value))
                    {
                        if (!tmp.Contains(each.ID))
                            continue;
                ...
                {
                    DataTable dt = tool._Q.Select(@"select $td_take.* from $td_take join discipline on $td_take.ref_discipline_id = discipline.id where '" +
                        dateTimeInput1.Value.ToString("yyyy/MM/dd") + "' <= discipline.register_date and discipline.register_date <= '" + dateTimeInput2.Value.ToString("yyyy/MM/dd") + "'");
                    foreach (DataRow row in dt.Rows)
                    {
                        DicDisciplineRegistByTeacher.Add("" + row["ref_discipline_id"], "" + row["ref_teacher_id"]);
                    }
